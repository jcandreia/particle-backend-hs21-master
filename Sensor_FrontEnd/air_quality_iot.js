...
function updateVariables(data) {
    ...
    if (data.eventName === "Empfehlung"){
        document.getElementById("empfevent").innerHTML = data.eventData;
    }
}
...
async function getEmpfehlung() {
    // request the variable "counter"
    var response = await axios.get(rootUrl + "/api/device/0/variable/Empfehlung");
    var Empfehlung = response.data.result;

    // update the html element
    document.getElementById("Empfehlung").innerHTML = Empfehlung;
}
